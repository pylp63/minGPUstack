...
class Server:
    ...
    async def _init_user(self, session: AsyncSession):
        ...
        # 二开: 初始管理员固定 admin/admin (内网部署约定), 不再生成随机
        # 密码、不强制首登改密. GPUSTACK_BOOTSTRAP_PASSWORD 仍可覆盖.
        bootstrap_password = self._config.bootstrap_password or "admin"
        require_password_change = False
        logger.info(
            "Bootstrap admin account: username=admin (二开版默认密码, "
            "可用 GPUSTACK_BOOTSTRAP_PASSWORD 覆盖)"
        )
