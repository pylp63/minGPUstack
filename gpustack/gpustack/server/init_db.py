...
async def init_db_engine(db_url: str):
    ...
    if db_url.startswith("postgresql://"):
        ...
        # 二开: support embedded SQLite for slim single-container
        # deployments (docker compose demo / small installs). The
        # sqlite view statements exist alongside the pg/mysql ones in
        # schemas/stmt.py (used by the test suite), so the worker view
