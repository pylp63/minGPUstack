...
class TenantContext:
    ...
    # Worker ids explicitly granted to the caller (user isolation,
    # feature one). Empty set means "no extra grants" (admin / SYSTEM
    # ignore it entirely). Populated lazily by the workers routes; kept
    # here so row-level visibility checks don't re-query per row.
    worker_grant_ids: Set[int] = field(default_factory=set)
...
def assert_resource_visible(
    ...
    *,
    ...
    if (
        ...
        # User isolation (feature one): an explicitly granted worker is
        # visible to the grantee even though it belongs to another
        # principal. ``worker_grant_ids`` is populated by the workers
        # routes; other resources keep the plain owner check.
        if (
            getattr(resource, "worker_id", None) is None
            and type(resource).__name__ == "Worker"
            and getattr(resource, "id", None) in ctx.worker_grant_ids
        ):
            return
        raise NotFoundException(message=not_found_message)
