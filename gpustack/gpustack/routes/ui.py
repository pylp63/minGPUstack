...
def register(app: FastAPI):
    ...
    # 二开: 自带管理控制台 (GPU 申请/审批/节点授权/一键部署架构).
    # 单文件页面, 数据全部走已鉴权的 /v2 API — 未登录用户打开会被页面
    # 内的 /users/me 401 重定向到主界面登录页, 因此此处无需再做鉴权.
    console_dir = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "ui_console"
    )
    _console_index = next(
        (
            name
