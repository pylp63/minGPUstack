...
def _make_worker_visibility_filter(ctx):
    ...
    Additionally, non-admin users with explicit worker-level grants
    (user isolation, feature one) also see the workers the admin
    opened to them — resolved from the per-request grant set the list
    handler injects via ``ctx`` (``request.state.worker_grant_ids``)."""

    grant_ids = getattr(ctx, "worker_grant_ids", None)
    ...
    def _visible(w) -> bool:
        ...
        if bypass_tenant_filter(ctx):
            return True
        if grant_ids is not None and w.id in grant_ids:
            return True
...
async def get_workers(
    ...
    # User isolation (feature one): a non-admin caller also sees workers
    # the admin explicitly granted them. Load the grant set once per
    # request and widen both the SQL conditions and the row filter.
    ...
    if not (bypass_tenant_filter(ctx) or cluster_scoped_system(ctx)):
        ...
        if grant_ids:
            ctx.worker_grant_ids = grant_ids
            from sqlalchemy import or_

            # Widen the owner-only condition to (owner OR granted ids).
            widened = [
                or_(
                    cond,
                    Worker.id.in_(grant_ids),
                )
                for cond in extra_conditions
            ]
            # tenant_list_conditions with no context (admin) yields [];
            # then grant ids alone scope the list.
            if not widened:
                widened = [Worker.id.in_(grant_ids)]
            extra_conditions = widened
...
async def get_worker(
    ...
    # User isolation (feature one): load grants so a granted worker is
    # visible to its grantee on the detail route too.
    from gpustack.schemas.worker_access import granted_worker_ids_for

    if not (bypass_tenant_filter(ctx) or cluster_scoped_system(ctx)):
        ctx.worker_grant_ids = set(
            await granted_worker_ids_for(session, ctx.user.id)
        )
