...
def public_maas_integration_upgrade():
    ...
    # 二开: `||` 而非 `concat()` — sqlite 没有 concat 函数; PG/MySQL 均支持 `||`
    # (MySQL 8 默认 sql_mode 下 || 仍是逻辑或, 但该迁移链只在新装 sqlite 库上重放,
    #  MySQL 用户由历史版本升级时 alembic_version 已超过该版本不会重放)
    conn.execute(sa.text("""
    insert into model_route_targets (name, route_id, model_id, route_name, weight, state)
        select m.name || '-deployment' as name,
            ma.id as route_id,
            m.id as model_id,
